...
/**
 * function to trigger welcome message
 */
function sendWelcomeMessage() {
  const welcomeMessage = "Halo, saya ShopMate! silahkan ketik 'mulai' untuk memulai percakapan";
  const botResponse = getBotResponse(welcomeMessage);
  $(botResponse).appendTo(".chats").hide().fadeIn(1000);
  scrollToBottomOfResults();
}

// call the sendWelcomeMessage function when the page is loaded
$(document).ready(() => {
  sendWelcomeMessage();
});
...
function restartConversation() {
  // Disable user input during the restart process
  $("#userInput").prop("disabled", true);
  ...
  // Clear the chat and user input
  $(".chats").html("");
  $(".usrInput").val("");

  // Send the restart command to the bot
  send("/restart");

  // Wait for a moment before enabling the input again and showing the welcome message
  setTimeout(() => {
    $("#userInput").prop("disabled", false); // Re-enable input
    sendWelcomeMessage(); // Show the welcome message
